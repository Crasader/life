...
const float c_kBump = 1.0;
const vec4 c_ambinet = vec4(0.5, 0.5, 0.5, 1.0);
...
void main(void)
{
    ...
    if(c_kBump!=1.0)
    {
        ...
        normal=vec3(normal.x,normal.y,normal.z/c_kBump);
    ...
    gl_FragColor=texColor*vec4(vec3(c_ambinet + diffuse),diffuse.a);
}
